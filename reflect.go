...
// fieldInfo is a struct field that should be mapped to a sheet column, or vice-versa
...
func getFieldInfos(rType reflect.Type, parentIndexChain []int, parentKeys []string) []fieldInfo {
	...
	for i := 0; i < fieldsCount; i++ {
		...
		if fieldType.Kind() == reflect.Struct {
			// Structs that implement any of the text or cell marshaling methods
		...
		// if the field is an embedded struct, ignore the xlsx tag
		if currFieldInfo == nil {
			continue
		}
		...
		if field.Type.Kind() == reflect.Slice || field.Type.Kind() == reflect.Array {
			var arrayLength = -1
			// if the field is a slice or an array, see if it has a `xlsx[n]` tag
			...
			if field.Type.Elem().Kind() == reflect.Struct {
				...
				// if no special xlsx[] tag was supplied, just include the field directly
				if arrayLength == -1 {
...
func filterTags(tagName string, indexChain []int, field reflect.StructField) (*fieldInfo, []string) {
	...
	for _, fieldTagEntry := range fieldTags {
		trimmedFieldTagEntry := strings.TrimSpace(fieldTagEntry) // handles cases like `xlsx:"foo, omitempty, default=test"`
